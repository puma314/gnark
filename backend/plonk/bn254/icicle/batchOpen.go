...
func NewSRS(size uint64, bAlpha *big.Int) (*SRS, error) {
	...
	if bAlpha.Cmp(&bMOne) == 0 {
		...
		Execute(int(size), func(start, end int) {
...
func BatchOpenSinglePoint(polynomials [][]fr.Element, digests []Digest, point fr.Element, hf hash.Hash, pk ProvingKey, dataTranscript ...[]byte) (kzg.BatchOpeningProof, error) {

	// check for invalid sizes
	nbDigests := len(digests)
	if nbDigests != len(polynomials) {
		return kzg.BatchOpeningProof{}, ErrInvalidNbDigests
	}
	...
	for _, p := range polynomials {
		if len(p) == 0 || len(p) > len(pk.G1) {
			return kzg.BatchOpeningProof{}, ErrInvalidPolynomialSize
		}
	...
	for i := 1; i < len(polynomials); i++ {
		i := i
		Execute(len(polynomials[i]), func(start, end int) {
			var pj fr.Element
			for j := start; j < end; j++ {
